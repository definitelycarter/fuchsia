...
- **Entrypoint `push` shed is uncounted** (surfaced by the
  [stress harness](./engine-stress-testing.md)). A `push` shed at the entrypoint (full mailbox)
  bypasses the route counters, and `push`'s doc overclaims that Health records the outcome — the
  clean fix is `push` **returning the offer outcome** so the host's ingress can react. (The
  sibling gap the harness found — a transient-restart panic-loss going uncounted — is now closed
  by `Health::crashed`, bumped on every caught panic in the restart supervisor.)
