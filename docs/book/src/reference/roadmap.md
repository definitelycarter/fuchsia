...
| Lifecycle-gated routing | Gate message admission on each target's lifecycle accept-state (`Starting`/`Running`/`Draining`/`Stopped`) × origination-vs-continuation, so quiescence is a routing property, not a front-door privilege. Demotes `push`/`push_durable` to callers of an origination primitive (mint + `run` root + offer); generalizes Graceful Shutdown's entrypoint seal and Supervised Lifecycle's on-running seam; reinstates in-graph origination + source-actor pumps as gated primitives. See [RFC](../rfcs/lifecycle-gated-routing.md) (supersedes [in-graph origination](../rfcs/in-graph-origination.md)). | `fuchsia-engine`, `fuchsia-transport`, `fuchsia-runtime`, `fuchsia-actor`, host |
...
| Routing sheds on a full downstream mailbox (at-most-once) with no per-target backpressure option. [Lifecycle-gated routing](../rfcs/lifecycle-gated-routing.md) names the full-mailbox case (`Admission::Dropped`) and defers a **per-edge overflow policy** (`Shed` / `Backpressure` / `DeadLetter`) to its own follow-up RFC; `Backpressure` extends `push_durable`'s blocking send to internal hops. | Low (intentional for the conditioning path; the lossless route is the follow-up) |
