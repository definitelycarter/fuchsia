...
| `fuchsia-workflow` | Persisted workflow definitions and Slate-backed CRUD. `Workflow`/`Node`/`NodeDefinition` (`Builtin`\|`Component`), `BuiltinConfig`, `ComponentConfig`, `Edge`. No trigger concept — what fires a workflow is a consumer concern. | `slate-db`/`-store`/`-query`, `bson`, `serde`, `thiserror` |
