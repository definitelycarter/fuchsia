...
| `fuchsia-runtime` | `context` | The per-delivery `ActorContext` build in isolation (`context/per_message`) — the construction the recv→handle loop does for every message: `execution_id` from the correlation, `node_id` shared from the actor's stable id (both `Arc<str>` refcount bumps), `task_id` a bare `u64` counter. Times only the context build, so the allocation profile is visible (it now allocates nothing — `task_id` is a `u64`, rendered to `"task-N"` lazily only at a guest). |
